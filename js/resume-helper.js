(function () {
  "use strict";
  var limiter = PilotToolLimit.make("pilot_resume_helper_usage_v1", 20);
  function bulletize(raw, role) {
    ...
    return lines.slice(0, 8).map(function (l) {
      if (/^[A-Z]/.test(l) && /\b(led|built|improved|managed|designed|created|shipped|reduced|increased)\b/i.test(l)) return "• " + l;
      return "• " + l.charAt(0).toUpperCase() + l.slice(1) + (roleBit && l.length < 80 ? "" : "");
    }).join("\n");
  }
